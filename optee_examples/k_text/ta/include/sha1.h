...
static void uint32_pack_big (char *s, uint32_t u){
...
static void sha1_transform(uint32_t *buf, uint32_t const *in){
...
static void sha1_feed(SHA1Schedule *ctx, unsigned char inb){
...
static void sha1_init(SHA1Schedule *ctx){
	register unsigned int i = 0 ;
...
static void sha1_update(SHA1Schedule *ctx, char const *buf, unsigned int len){
	register unsigned int i = 0 ;
	for (; i < len ; i++) sha1_feed(ctx, (unsigned char)buf[i]);
}
static void sha1_final(SHA1Schedule *ctx, char *digest){
	char pack[8] ;
	register unsigned int i = 0 ;
...
/*
static void sha1(char*message,unsigned len,char*out){
