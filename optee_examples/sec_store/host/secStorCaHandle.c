...
//TEEC_UID uuid = TA_HELLO_WORLD_UUID;
TEEC_Context g_TaskContext; 
//TEEC_Result res;TEEC_Context ctx;res = TEEC_InitializeContext(NULL, &ctx)
...
static int l_CryptoVerifyCa_TaskInit(void)//初始化；对应Hello_world中的初始化部分
{
    TEEC_Result result;
    int l_RetVal = OK;
...
static int l_CryptoVerifyCa_OpenSession(TEEC_Session* session)
{
    TEEC_Result result;
    int l_RetVal = FAIL;
    uint32_t origin;//uint32_t err_origin;
...
static int l_CryptoVerifyCa_SendCommand(TEEC_Operation* operation, TEEC_Session* session, uint32_t commandID)
{
    ...
    result = TEEC_InvokeCommand(session, commandID, operation, &origin);//commandID为命令
    if (result != TEEC_SUCCESS) 
    {
...
int g_SecStorCa_CreateFile(UINT32 len, CHAR* fileName)
{
    ...
    l_operation.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INPUT,TEEC_NONE, 
                                              TEEC_NONE, TEEC_NONE);
    l_operation.params[0].tmpref.size = len;//形参传进来的
    l_operation.params[0].tmpref.buffer = fileName;//形参传进来的
...
int g_SecStorCa_RenameFile(UINT32 oldLen, CHAR* oldName, UINT32 newLen, CHAR* newName)
{
    ...
    /**5) The clean up operation */
    cleanup_3:
        TEEC_CloseSession(&l_session); //init ok open ok
    cleanup_2:
        TEEC_FinalizeContext(&g_TaskContext); //init ok open fail
    cleanup_1:
        return l_RetVal; //init fail
}
