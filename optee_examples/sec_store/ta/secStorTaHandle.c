...
TEE_Result store_data_sample(void)
{
    ...
    //TEE_CloseAndDeletePersistentObject(persistent_data); ////different?
    TEE_CloseObject(persistent_data);
...
{
    TEE_Result l_ret = TEE_EXEC_FAIL; //.h
    UINT32 l_AccFlg = TEE_DATA_FLAG_ACCESS_WRITE | TEE_DATA_FLAG_ACCESS_READ; //api
