...
int main(int argc,char **argv)
{
    ...
    vir = strtol(argv[1],NULL,16);
    mem_addr(vir, &phy);
    printf("virtual addr = %x , physical addr = %x\n", vir, phy);
    if (fd == -1)
    {
      printf ("open /dev/mem fail!\n");
      return (-1);
    }
    map_base = mmap (NULL, 0xff, PROT_READ | PROT_WRITE, MAP_SHARED, fd,phy);
  ...
  for (; i < 0xf; ++i)

    {
      ...
      printf ("address: 0x%lx   value: 0x%x\t\t", addr,
              ...
              (unsigned int) content);//这个地址我们不关心；关心内容
   }
  close (fd);
